...
var app = {
    ...
    formatDate: function(date, format) {
        ...
        s = s.replace('MM', pad(date.getMounth()+1));
...
function valueCalcDate(value, dataBlock) {
    ...
    var mounth = ((value & b('0b111100000000')) >> 8);
...
WMBUS.prototype.getAESKey = function (sn) {
    return (this.aesKey.hasOwnProperty('sn' + sn) ? this.aesKey['sn'+sn] : "");
};
...
WMBUS.prototype.removeCRC = function (msg) {
    ...
    var blocksize_with_crc = cc.LL_BLOCK_SIZE + cc.CRC_SIZE;
    var crcoffset;
    ...
    app.log.debug("LÃ¤nge " + msgLen + " Anz. BlÃ¶cke " + noOfBlocks + " rest " + rest);

    for (var i = 0; i < noOfBlocks; i++) {
        crcoffset = blocksize_with_crc * i + cc.LL_BLOCK_SIZE;
        app.log.debug(i + ': crc offset ' + crcoffset);
        if (rest > 0 && crcoffset + cc.CRC_SIZE > (noOfBlocks - 1) * blocksize_with_crc + rest) {
        ...
        crc = this.unpack('n', msg.substr(crcoffset, cc.CRC_SIZE));

        app.log.debug(i + ': CRC ' + crc.toString(16) + ', calc ' + crc.build(msg.substr(blocksize_with_crc * i, blocksize)).toString(16) + 'blocksize ' + blocksize);
        //if (crc != this.checkCRC(msg.substr(blocksize_with_crc * i, blocksize))) {
        if (crc != crc.build(msg.substr(blocksize_with_crc * i, blocksize))) {
...
WMBUS.prototype.decodeConfigword = function () {
    ...
    this.cw_parts.bidirectional = this.cw & b('0b1000000000000000') >> 15;
    this.cw_parts.accessability = this.cw & b('0b0100000000000000') >> 14;
    this.cw_parts.synchronous = this.cw & b('0b0010000000000000') >> 13;
    this.cw_parts.mode = this.cw & b('0b0000111100000000') >> 8;
    this.cw_parts.encrypted_blocks = this.cw & b('0b0000000011110000') >> 4;
    this.cw_parts.content = this.cw & b('0b0000000000001100') >> 2;
    this.cw_parts.repeated_access = this.cw & b('0b0000000000000010') >> 1;
    this.cw_parts.hops = this.cw & b('0b0000000000000001');
...
WMBUS.prototype.decrypt = function (encrypted) {
    ...
    const decipher = crypto.createDecipheriv('aes-128-cbc', this.getAESKey(this.afield_id), initVector, {});
...
function decodeDataInformationBlock (dib, dataBlockRef) {
    ...
    dataBlockRef.storageNo = (dif & b('0b01000000')) >> 6;
    dataBlockRef.functionField = (dif & b('0b00110000')) >> 4;
    dataBlockRef.functionFieldText = functionFieldTypes[dataBlockRef.functionField];
    dataBlockRef.dataField = dif & b('0b00001111');
    ...
    while (isExtension) {
        ...
        dataBlockRef.storageNo |= (dif & b('0b00001111')) << (difExtNo * 4) + 1;
        dataBlockRef.tariff |= ((dif & b('0b00110000') >> 4)) << ((difExtNo - 1) * 2);
        dataBlockRef.devUnit |= ((dif & b('0b01000000') >> 6)) << (difExtNo - 1);
        ...
        while (offset < payload.length) {
            ...
            switch (dataBlock.dataField) {
                ...
                case cc.DIF_FLOAT32:
                    ...
                    break;
                case cc.DIF_VARLEN:
                    var lvar = this.unpack('C', payload.substr(offset++, 1));
...
WMBUS.prototype.decodeApplicationLayer = function () {
    if (this.crcRemoved) {
        ...
        var applicationlayer = this.removeCRC(this.msg.substr(12));
    ...
    switch (this.cw_parts.mode) {
        ...
        case 5: // AES Cipher Block Chaining Mode(AES - CBC) with dynamicinitialization vector
            ...
            if (this.unpack('n', payload) != 0x2f2f) {
                ...
                app.log.debug(this.unpack('n', payload).toString(16));
                return 0;
            }
            this.decrypted = 1;
...
WMBUS.prototype.decodeLinkLayer = function (linklayer) {
    ...
    if (!this.crcRemoved) {
        app.log.debug("crc0 " + this.crc0.toString(16) + " calc " + crc.build(linklayer.substr(0, 10)).toString(16));

        //if (this.crc0 != this.checkCRC(linklayer.substr(0, 10))) {
        if (this.crc0 != crc.build(linklayer.substr(0, 10))) {
    ...
    this.datalen = this.lfield - 9; // this is without CRCs and the lfield itself
    this.datablocks = parseInt(this.datalen / cc.LL_BLOCK_SIZE);
    if (this.datalen % cc.LL_BLOCK_SIZE != 0) this.datablocks++
    this.msglen = 12 + this.datalen + this.datablocks * cc.CRC_SIZE;
