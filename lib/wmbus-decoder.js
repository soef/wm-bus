...
WMBUS.prototype.removeCRC = function (msg) {
    ...
    for (var i = 0; i < noOfBlocks; i++) {
        ...
        _crc = this.unpack('n', msg.substr(crcoffset, cc.CRC_SIZE));

        app.log.debug(i + ': CRC ' + _crc.toString(16) + ', calc ' + crc.build(msg.substr(blocksize_with_crc * i, blocksize)).toString(16) + 'blocksize ' + blocksize);
        //if (crc != this.checkCRC(msg.substr(blocksize_with_crc * i, blocksize))) {
        if (_crc != crc.build(msg.substr(blocksize_with_crc * i, blocksize))) {
